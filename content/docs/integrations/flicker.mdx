...
* Installers for Windows, macOS, and Linux are published from the [GitHub Releases page](https://github.com/EmberlyOSS/Flicker/releases), but are not yet code-signed you may see an OS warning ("unknown publisher" on Windows, "unidentified developer" on macOS) when installing. 
* You can also build from source. See [Getting Flicker](#getting-flicker) below and the full [Flicker Development Guide](/docs/integrations/flicker-development) for details.
</Callout>
