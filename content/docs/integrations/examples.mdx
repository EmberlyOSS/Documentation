...
async function addCollaborator(fileId, userEmail, role = "EDITOR") {
...
addCollaborator("cm7xabc123", "alice@example.com", "EDITOR");
```
...
File uploads and most API endpoints aren't rate-limited by plan — only a handful of security-sensitive endpoints (registration, password reset, contact form, domain verification) have flat, IP-scoped limits. See the [User API Reference](/docs/api/user-reference#rate-limits) for the full list. A limited endpoint returns `429 Too Many Requests` with a `Retry-After` header when exceeded.
