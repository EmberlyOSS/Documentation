...
# This:
# 1. Starts Vite dev server on localhost:5173 (via beforeDevCommand)
...
Plain `bun run dev` only starts the Vite frontend server on its own — it won't launch the desktop window.

**First startup** may take 2-3 minutes as Rust builds. Subsequent runs are faster using incremental compilation.

### Development Window

When `bun run tauri:dev` succeeds:
...
```bash
# Full build (React + Rust), produces installers
bun run tauri:build

# Outputs to: src-tauri/target/release/bundle/
```

`bun run build` on its own only runs `tsc && vite build` — the frontend type-check and bundle, with no Rust compilation or installer output. Use `bun run tauri:build` to actually produce a runnable app.

### Platform-Specific Builds

Cross-compiling for a different OS than the one you're building on isn't supported (Tauri needs each platform's native webview toolchain) — each of these must run on that actual platform, which is why the project's [release workflow](https://github.com/EmberlyOSS/Flicker/blob/master/.github/workflows/release.yml) uses a matrix of Windows/macOS/Ubuntu runners:
...
# macOS Apple Silicon (must run on macOS)
...
After `bun run tauri:build`, installers appear in:
...
There is no test runner configured yet — `package.json` has no `test` script, and no test suite currently exists in the frontend. The snippet below is illustrative of the intended setup (Vitest + Testing Library) for when one is added, not a command you can run today.
...
interface UploadAreaProps {
  ...
  password?: string;
  behavior?: BehaviorConfig; // post-upload action, clipboard format
}
```
...
**Features:**
- Tabbed interface: Account, Capture, Hotkeys, Preferences (appearance/theme + sounds), About
...
- [support@embrly.ca](mailto:support@embrly.ca)

---

## Contributing

See the [Contributing Guide](https://github.com/EmberlyOSS/Flicker/blob/master/CONTRIBUTING.md) in the Flicker repository (separate from the main Emberly website's contributing docs) for full guidelines on:
