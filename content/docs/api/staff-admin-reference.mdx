...
Send an email to multiple users. Full request body and response are documented once under [Email Operations](#email-operations) further down this page — see that section for the current fields (`subject`, `body`, `priority`, `ctaLabel`, `ctaHref`, `recipientFilter`, `dryRun`).
...
Paginated audit log of all admin and system events. Full details, including the response shape, are documented once under [Audit Logs](#audit-logs-1) further down this page.
...
**Query Parameters:**
- `search` — Free-text search across actor/target email, type, action, resource, IP
...
- `auditableOnly` — `true` to restrict to events flagged as auditable
...
```json
{
  ...
  "pagination": { "total": 1, "page": 1, "limit": 20, "pageCount": 1 }
}
```
...
```json
{ "success": true, "isDryRun": true, "recipientCount": 15420, "message": "Would send to 15420 recipients" }
```

Otherwise, sends in batches of 50 and returns:
```json
{
  "success": true,
  "sentTo": 15418,
  "total": 15420,
  "errorCount": 2,
  "errors": ["Failed to send to bounced@example.com: ..."],
  "message": "Sent to 15418 of 15420 recipients"
}
```
