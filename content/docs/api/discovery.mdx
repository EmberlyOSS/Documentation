...
<Callout type="warn">
  **Cloud only.** Nexium/Discovery runs on embrly.ca and these endpoints return `404` on self-hosted instances see [Cloud vs. Self-Hosted](/docs/self-hosting/cloud-vs-self-hosted).
</Callout>
...
|-------|------|----------|-------------|
| `name` | string | Yes | 3-60 characters |
| `description` | string | No | Up to 500 characters |
| `skills` | string[] | No | Up to 20 skill tags |
| `maxSize` | number | No | Member cap, 2 to the squad-size limit for your plan |
| `isPublic` | boolean | No | Whether the squad is listed publicly |
...
**Add a member (owner):**
```json
{ "action": "add", "userId": "user_456", "role": "MEMBER" }
```
...
**Change role (owner):**
```json
{ "action": "setRole", "userId": "user_456", "role": "OBSERVER" }
```

**Available roles:** `OWNER`, `MEMBER`, `OBSERVER`
