...
| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/api/files/[id]/collaborators` | List a file's collaborators |
| `POST` | `/api/files/[id]/collaborators` | Add a collaborator |
| `DELETE` | `/api/files/[id]/collaborators/[collaboratorId]` | Remove a collaborator |
| `GET` | `/api/files/[id]/collaborators/me` | Get your own collaborator role on a file |
| `GET` | `/api/files/[id]/suggestions` | List edit suggestions on a file |
| `GET` | `/api/files/suggestions/pending` | List pending suggestions across all your files |

See [File Collaboration](/docs/user-guide/file-collaboration) for the full collaborator/suggestion workflow and role details (`EDITOR` / `SUGGESTER`).
...
3. VirusTotal hash scan (skipped for images, video, and audio — see [VirusTotal Scanning](/docs/security/virustotal) for why)
