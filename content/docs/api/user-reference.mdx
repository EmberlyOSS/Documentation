...
Rate limits are not tiered by plan — they're flat, IP-scoped limits applied to a handful of specific security-sensitive endpoints, not a general per-plan API quota:

| Endpoint | Limit |
|----------|-------|
| `POST /api/auth/register` | 5 per 60s per IP |
| `POST /api/auth/password/forgot` | 1 per 600s per IP |
| `POST /api/auth/password/reset` | 5 per 600s per IP |
| `POST /api/auth/resend-verification` | 3 per 600s per IP |
| `POST /api/contact` | 3 per 60s per IP |
| `POST /api/auth/desktop` (failed attempts) | 2 per 600s |
| `POST /api/domains/[id]/cf-check` | 10 per 10 minutes |

Endpoints not listed here (file uploads, listing files, URL shortening, etc.) have no request-rate limit today — only the storage/upload-size caps for your plan apply. On the limited endpoints above, a `429 Too Many Requests` is returned with a `Retry-After` header (seconds until the limit resets).
...
Add a collaborator to a file. Provide either `userEmail` or `userUrlId` to identify the person.

**Request Body:**
```json
{
  "userEmail": "alice@example.com",
  "role": "EDITOR" // or "SUGGESTER" (default: EDITOR)
}
```
...
```json
{
  ...
  "maxSize": 10, // 2 up to your plan's squad-size limit
  "isPublic": true
}
```
...
<Callout type="info">
  Squad roles are `OWNER`, `MEMBER`, and `OBSERVER` — there is no separate `ADMIN` or `EDITOR` squad role.
</Callout>
...
URL shortening is documented on its own page: see [URL Shortener API](/docs/api/urls). It lives at `/api/urls` (not `/api/shorturl`), the request body is just `{ "url": "..." }`, short codes are auto-generated (6-character, no custom/vanity slug support), and there's no password option on shortened URLs.
