...
<Callout type="warn">
  **Cloud only.** Custom domains run on embrly.ca's managed Cloudflare integration and aren't available on self-hosted instances — see [Cloud vs. Self-Hosted](/docs/self-hosting/cloud-vs-self-hosted).
</Callout>
...
| Type | Target / Value | TTL |
|-----------|----------------------|-----|
| **CNAME** | `cname.embrly.ca` | 300 |
...
For more detail see the [Domain API reference](/docs/api/domains).
