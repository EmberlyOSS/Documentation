...
Edit `.env` with your database/Redis connection strings, `NEXTAUTH_URL`/`NEXT_PUBLIC_BASE_URL` (your domain), a generated `NEXTAUTH_SECRET`, and OAuth credentials. See [Environment Variables](/docs/self-hosting/environment) for the full reference — storage and email are configured later via the Admin Panel, not `.env`.
...
The app listens on port 3000 by default. Open `http://YOUR_SERVER_IP:3000` and complete the **Setup Wizard** to create your admin account — see [First-Time Setup](/docs/self-hosting/first-time-setup) for a step-by-step walkthrough.
...
You already chose local or S3-compatible storage during the Setup Wizard's Storage step (see [First-Time Setup](/docs/self-hosting/first-time-setup)). To switch providers later, or fill in the public URL and other advanced fields, go to **Admin Panel → Settings → Integrations → Storage**:
