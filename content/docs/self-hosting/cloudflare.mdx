...
Cloudflare credentials are **not** environment variables — they're configured in the database via **Admin Panel → Settings → Integrations → Cloudflare**:

- API token (Zone: Read, Custom Hostnames: Edit)
- Account ID
- Zone ID

This matches how the other third-party integrations (Stripe, Resend/SMTP, object storage providers) are configured — see [Environment Variables](/docs/self-hosting/environment) for the full split between what's an env var and what's Admin Panel-configured.

Also configure a CNAME record in Cloudflare pointing your chosen target hostname (e.g. `cname.yourdomain.com`) to your app's hostname — this is the value users will be told to point their own CNAME records at.

### How It Works

When a user clicks **Verify** in the dashboard:

1. Emberly performs a DNS lookup to confirm the CNAME points to your configured target hostname
